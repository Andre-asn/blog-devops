...
echo "   cat ~/.ssh/github_actions  # Copy this for GitHub secrets"
echo ""
echo "2. Add to GitHub repository secrets:"
echo "   DIGITALOCEAN_SSH_KEY: <private key from above>"
echo "   DIGITALOCEAN_HOST: $(curl -s ifconfig.me)"
echo "   DIGITALOCEAN_USER: root"
echo "   APP_DIRECTORY: /root/blog-app"
echo "   MONGO_URI: mongodb://localhost:27017/"
echo "   SECRET_KEY: <generate a new one for GitHub>"
echo "   DATABASE_NAME: blog_db"
echo ""
...
if sudo systemctl is-active --quiet blog-app && [ "$HEALTH_STATUS" = "200" ]; then
    echo "🎉 Droplet 1 setup successful! Application is running and responding."
    echo ""
